...
#Creamos el exchange 'donbot' de tipo 'fanout'
...
def message(payload):
    ...
    if text.startswith("[traducir]"):
        channel.basic_publish(exchange='donbot', routing_key="traducir", body=text)

    if text.startswith("[wikipedia]"):
        channel.basic_publish(exchange='donbot', routing_key="wikipedia", body=text)
...
if __name__ == "__main__":
    ...
    # Run our app on our externally facing IP address on port 80 instead of
