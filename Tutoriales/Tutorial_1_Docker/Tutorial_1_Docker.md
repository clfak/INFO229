...
* **Imagen:** corresponde a una especie de plantilla, estas pueden incluir una aplicación, los binarios y librerías necesarias para la construcción de los contenedores, es decir las imágenes se utilizan para crear contenedores.

* **Contenedor:** en *Docker* los contenedores son imágenes *Docker* en ejecución.

2. ¿Cuál es la diferencia entre el archivo Dockerfile y el archivo Docker-compose.yml?

* **Dockerfile:** es un archivo de texto en el cual contiene instrucciones para la creación de la imagen de una aplicación.

* **Docker-compose.yml:**  conjunto de scripts que facilitan la construcción de los servicios, *docker-compose* provee al engine un esquema para realizar determinadas tareas(escritas en lenguaje *yaml*).

	Mientras que el *Dockerfile* define como crear una imagen, *docker-compose* permite vincular y configurar los contenedores en conjunto para construir varios servicios.

3. ¿Si utilizo un contenedor que contiene una Base de Datos (Redis, Mysql u otro), cómo y dónde se guardan los datos?

	Una buena práctica es guardar estos datos  dentro de  volúmenes que  son  carpetas compartidas entre contenedores. Los volúmenes se crean fuera de los contenedores, ya que si estos se borran los datos persisten.


4. ¿Qué es un *port*? ¿Por qué algunas imágenes requieren hacer un *bind* entre distintos puertos?

	*Port* se refiere a los puertos, su función es comunicar elementos entre sí, recibiendo y enviando datos.
